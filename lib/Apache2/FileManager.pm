...
sub handler {
  ...
  my $obj = $package->new();
  r->content_type('text/html');
  r->print("<HTML><HEAD><TITLE>" . r->hostname . " File Manager $VERSION</TITLE></HEAD>");
  $obj->print();
  r->print("</HTML>");
...
sub handler_noprint {
  ...
  my $obj = $package->new();
  r->content_type('text/html');
  my $retval = "<HTML><HEAD><TITLE>" . r->hostname . " File Manager $VERSION</TITLE></HEAD>";
  $retval .= $obj->noprint();
  $retval .= "</HTML>";
  return $retval;
}
...
sub intialize {
  ...
  my $curr_dir = r->param('FILEMANAGER_curr_dir');
  if (defined $curr_dir) {
#      print {*STDERR} 'have pre-regex  $curr_dir = ', q{'}, $curr_dir, q{'}, "\n";
      $curr_dir =~ s/\.\.//g; 
      $curr_dir =~ s/^\///;
      $curr_dir =~ s/\/$//;
#      print {*STDERR} 'have post-regex $curr_dir = ', q{'}, $curr_dir, q{'}, "\n";
  }
  else {
      $curr_dir = q{};
  }
...
sub html_javascript {
  ...
  $cookie_name .= "_FM";
  my $uri = r->uri;
#  print {*STDERR} 'have $uri = ', q{'}, $uri, q{'}, "\n";
  my $curr_dir = r->param('FILEMANAGER_curr_dir');
  if (not defined $curr_dir) {
      $curr_dir = q{};
  }
  else {
      $curr_dir .= '/'
...
#  print {*STDERR} 'have $curr_dir = ', q{'}, $curr_dir, q{'}, "\n";
  ...
  return "
    ...
    function print_upload () {
      ...
      d.write(\"<HTML><BODY><CENTER><H1>Upload Files</H1>\"+
              \"<FORM NAME=UploadForm ACTION='" . $uri . "' \"+
...
sub html_hidden_fields {
  ...
  # NEED ANSWER: should the following code be enabled?
#  else {
#      $curr_dir .= '/'
#  }

  my $last_select_all = r->param('FILEMANAGER_last_select_all');
  if (not defined $last_select_all) {
      $last_select_all = q{};
  }
